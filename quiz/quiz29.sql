...
DELETE FROM 테이블명; -- 테이블을 삭제한다
TRUNCATE TABLE 테이블명; -- 테이블 내 존재하는 데이터를 삭제한다
;
...
(
    ...
    CONSTRAINT PK_MEMBER_POINT PRIMARY KEY (id, no)
);
...
SELECT
    ...
    (SELECT NVL(MAX(no), 0) + 1 FROM T_MEMBER_POINT WHERE member_id = 'A'), 
    'A', 
    30, 
    SYSDATE 
FROM DUAL;
...
SELECT
    T_MEMBER_POINT_PK_SEQ.nextval, 
    (SELECT NVL(MAX(no), 0) + 1 FROM T_MEMBER_POINT WHERE member_id = 'B'), 
    'B', 
    40, 
    SYSDATE 
FROM DUAL;
...
SELECT
    T_MEMBER_POINT_PK_SEQ.nextval, 
    (SELECT NVL(MAX(no), 0) + 1 FROM T_MEMBER_POINT WHERE member_id = 'C'), 
    'C', 
    40, 
    SYSDATE 
FROM DUAL;
